...
##### What does “relaxing” mean here?
In this context, **“relaxing”** does not mean “turn off security” or “make the cluster less secure.” It means **loosening the default assumption that `istiod` only needs to talk to workloads inside the same cluster**.

Think of it like this:
- **Default behavior:** `istiod` is configured as if every sidecar/ztunnel and webhook client is local to the cluster.
- **Relaxed behavior:** `istiod` is allowed to accept connections from remote clusters over the network, as long as the identity and TLS trust are still valid.

A simple example:
- In a single-cluster setup, a sidecar in `cluster1` only needs to talk to `istiod` in `cluster1`.
- In a Primary-Remote setup, a sidecar in `cluster2` must talk to `istiod` in `cluster1` over the WAN/VPN.
- To make that possible, Istio must **relax** the old “cluster-local-only” assumption and expose the control plane endpoints (`15012`, `15017`) to trusted remote clients.

So the word **relaxing** here means:
- **relax the topology assumption**
- **widen the trust boundary slightly**
- **keep mTLS and identity validation intact**

In short: **relaxing = allowing the control plane to serve remote proxies, while still preserving the security model.**
...
```
* **Why do this?** Standard Ingress Gateways `North-South` handle public internet traffic. `East-West` Gateways handle internal cross-cluster (datacenter-to-datacenter) traffic.
* **Under the hood:**
