...
spec:
  ...
  # The controllerName: istio.io/gateway-controller is a unique string that identifies the Istiod controller.
  # It watches the API Server for Gateways with this class and reconciles them by programming the selected Envoy gateway (the dataplane).
...
### What is an "Unmanaged" Gateway? (The Remote Scenario)

The `istio-remote` class uses a special controller name: `istio.io/unmanaged-gateway`. This is used exclusively in **Multi-Cluster** or **Cross-Network** setups.

**Key Difference:**
- **Standard (`istio`):** Provisions infrastructure (Creates Pods, Services, LBs).
- **Remote (`istio-remote`):** Provisions **NOTHING**. It is a "Reflective" gateway.

In a Primary-Remote setup, you have one "Brain" (istiod in Cluster A) and one "Worker" (Cluster B). Cluster B doesn't have its own `istiod` but it has `istiod-remote`, it relies on Cluster A for everything.

**Why use this?**
In a Multi-Cluster mesh, Cluster A (Primary) needs to know about the Gateway in Cluster B (Remote) to route traffic to it. You create a `Gateway` in Cluster A that points to the *existing* IP of Cluster B. Since Cluster B already has the pods running, Cluster A should not try to spin up new ones.
...
metadata:
  name: istio-remote
spec:
  controllerName: istio.io/unmanaged-gateway   # ← Instruction to istiod: "Watch this, but don't create pods"
  description: Remote to this cluster. Used to represent another cluster's Gateway IP.
```
...
### Mode 6: AUTO_PASSTHROUGH (Multi-Cluster Backbone)

The `AUTO_PASSTHROUGH` mode is specialized for routing traffic between Istio clusters. Unlike standard `PASSTHROUGH`, it is completely **Stateless** — it does not need any routing rules to be defined in a `VirtualService`.
This is useful for **multi-cluster** or **global mesh** scenarios where you want to route traffic to a service in another cluster without having to define a `VirtualService` for every possible destination.

**How it works:**
1. **SNI as a Data Packet:** When a pod in Cluster A wants to talk to a pod in Cluster B, it sends a TLS packet where the **SNI header** is formatted like a magic string: `outbound_.80_._.myservice.namespace.svc.cluster.local`.
2. **Gateway Logic:** The Ingress Gateway in Cluster B receives this on port `15443`. Because the mode is `AUTO_PASSTHROUGH`, it says: *"I don't need to look at a VirtualService. I will simply parse that SNI string, extract the service name (`myservice`) and port (`80`), and forward the encrypted bytes directly to the Kubernetes Service FQDN."*
3. **No Termination:** Like `PASSTHROUGH`, the Gateway **cannot** see the data inside. It only sees the SNI label on the outside of the "envelope."

**Key Comparison:**

| Feature | `PASSTHROUGH` | `AUTO_PASSTHROUGH` |
| :--- | :--- | :--- |
| **VirtualService Required?** | **YES** (to map SNI to Host) | **NO** (Parsing is automatic) |
| **SNI Content** | Usually a DNS name (`app.com`) | Istio internal name (`outbound_.80...`) |
| **Typical Use Case** | Serving legacy TLS apps | **Multi-cluster** / **Global Mesh** |

```yaml
# Like PASSTHROUGH, but does NOT require a VirtualService.
# The Envoy proxy reads the destination from the SNI value directly
# (e.g. "outbound_.80_._.hello.default.svc.cluster.local")
...
spec:
  ...
  servers:
  - port:
      number: 15443                   # The standard port for inter-cluster traffic
      name: tls-auto
      protocol: TLS
    tls:
      mode: AUTO_PASSTHROUGH          # SNI is treated as a routing instruction
    hosts:
    - "*.local"
```
